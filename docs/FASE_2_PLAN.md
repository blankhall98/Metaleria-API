...
| 4 (capital diario) | ✅ En prod (07-ago, Ronda E) | Migración `a9b8c7d6e5f4`: `moneda` en cuentas Scrap360 (USD entra al capital con TC manual, sin TC no suma y se advierte), `capital_ajustes_manuales` (comodín con reversa compensatoria) y `capital_snapshots` (fotos por día y alcance — el histórico funciona como el Excel de la clienta: la utilidad del período es la resta entre dos fotos, no una reconstrucción de ledgers). El efectivo sale del último corte CERRADO por sucursal. Fusión de sucursales vía checkboxes (`?suc=1&suc=3`). Guardado por `scripts/test_capital.py` (10 checks) |
...
| 2026-08-07 | **Ronda E.2: tratos de contenedores en prod** (commit `90cc9ee`, migración `d4e5f6a7b8c9`) | Punto 3 completo: cadena de cálculo del Excel verificada al centavo en pantalla ($2,762,451.56 del caso de prueba), vista previa en vivo, TC ponderado en pagos partidos, EBONY directo; `scripts/test_tratos.py` 28/28. El diccionario de acentos perdió el par estas→estás (ambiguo: rompía el demostrativo) |
| 2026-08-07 | **Ronda E.3: capital diario en prod — LA FASE 2 QUEDA COMPLETA** | Punto 4: moneda USD + TC manual, comodín con reversas, fotos de capital por día/alcance con utilidad entre fotos, fusión de sucursales, efectivo desde cortes cerrados; `scripts/test_capital.py` 10/10. Los 13 puntos del documento de la clienta están ✅ En prod |
| 2026-08-07 | **Cierre fino contra la letra del documento** (releído completo de Drive) | Dos matices cerrados: (1) la nota de venta ahora elige a qué trato abona desde su propio detalle ("cuando se haga la nota… elegir cuál orden se vende") — se puede vincular desde borrador y los kilos cuentan al aprobarse; (2) `/capital` abre por defecto con base "promedio de compra" ("con el precio actual de compra sin tener que ponerlo"). Queda a validar con la clienta: "eliminar sucursales" se entregó como archivar con guardas, y la etiqueta "Saldo inicial" en el detalle de Cuentas Scrap360 (comentario de junta, pantalla ya oculta del menú) |
| 2026-08-07 | **Las dos decisiones pendientes, cerradas** | (1) "Eliminar sucursales" = archivar con guardas queda validado como suficiente — decisión tomada, ya nada que preguntar en la junta. (2) "Saldo inicial" en Cuentas Scrap360 se renombró a **"Saldo de apertura"** en todo el módulo: banda de KPIs del detalle, lista (sub y columna), formulario, reportes Excel/PDF, mensajes de error y comentario del movimiento de apertura de cuentas nuevas. El "Saldo inicial" del corte de caja NO se tocó (concepto distinto: efectivo con que abre la caja del día). Verificado: import OK, acentos 0 pendientes, 4 rutas de cuentas en 200 con el rótulo nuevo |
| 2026-08-07 | **Pulido táctil móvil + tablet, pantalla por pantalla (§22 de scrap360.css) — el escritorio no cambió ni un pixel** | Auditoría de las 47 vistas × 4 anchos (390/768/1024/1440) con capturas y chequeo mecánico propio. Correcciones, todas dentro de media queries max-width al final de la hoja: (a) bug de cascade — los controles `-sm` anulaban la regla móvil de 44px (formularios de abono, exportadores, filtros: quedaban en 30px en el teléfono); (b) capa táctil de tablet 768–1023.98 que no existía: inputs 44px, botones 44/40, casillas 22px, enlaces de celda con relleno-margen negativo, apriete extra del modo compacto (notas 10 col pasó de +68px a +28px de scroll); (c) modo tarjeta: el menú "⋯" era un botón de 30px huérfano (el `width:1%` de escritorio ganaba el cascade) — ahora la fila de acciones se estira completa; las celdas con input apilan como bloque (el kg del editor quedaba aplastado a pixeles); el pie de totales ya no desborda; enlaces de tarjeta a 36px de alto; (d) bandas de indicadores sin celda viuda con hueco ≤991.98. Métricas del chequeo mecánico en anchos táctiles: objetivos chicos 331→~20 (restan casillas de 22px con etiqueta y "#9" de 29×36), inputs bajos 174→0, overflow de página 0 antes y 0 después. **Prueba de no-regresión de escritorio: 47/47 vistas byte-idénticas en 1440 y en 1024 (CSS viejo vs nuevo, animaciones deshabilitadas)**. Gates: check_ui 84 vistas sin incumplimientos, acentos 0, rutas 200 |
...
| 2026-09-04 | **Solicitudes de septiembre (post-fase 2): kilos por material** — registro en `docs/SOLICITUDES_SEP_2026.md` | Tres puntos de la clienta en prod el mismo día, un commit por punto: (1) tarjeta "Kilos por material" con rango propio en el expediente del socio + hoja nueva en el Excel; (2) reporte `/reporte-materiales`, ranking de proveedores por kilos de un material y período, con Excel; (3) estado de cuenta desglosado por material al estilo de su hoja (pantalla y Excel con 13 columnas, PDF con línea compacta), saldo corrido intacto. Servicio `kilos_material_service` con 13 pruebas nuevas (`tests/test_kilos_material.py`) |
